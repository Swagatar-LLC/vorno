...
describe('mid-stream queue runtime invariants', () => {
  ...
  it('queues attachments and semantic options intact without calling a text-only redirect', async () => {
    ...
    // Leading args only: the fork's `processNextQueuedMessage` passes extra
    // trailing params upstream's does not (optimistic id, rpc context, page
    // callback, and the claimed send admission that keeps a replay visible to a
    // shutdown). The payload identity this test is about is the first six.
    expect(replay.mock.calls[0]!.slice(0, 6)).toEqual([managed.id, 'inspect image', attachments, storedAttachments, options, managed.messages[0]!.id])
  })
