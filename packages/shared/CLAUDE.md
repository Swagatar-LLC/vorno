...
- **Page action runtime authority** (SUV-0065, ADR-0033). Every Page action is admitted by the host, never by the caller. `PageActionOrigin` / `PageActionAuthority` (`@craft-agent/core`) are **arguments, not request fields** — a caller that could name its own origin would name the most privileged one. Both policy tables in `pages/types.ts` are **mapped over their unions** (`PAGE_ACTION_MUTATION`, `PAGE_ACTION_ORIGIN_POLICY`), so a new descriptor kind or origin fails to compile until it states its policy, and an unattributed caller has no entry at all. Only `api GET` is non-mutating; unknown kinds fail closed as mutating. Mutating actions need a host-minted, single-use, ≤10s activation ticket bound to a canonical request hash **and** the approved descriptor signature, plus host-rendered first-use confirmation per render for every mutating kind (ADR-0033 §3 names only script/session, but the `roadmap/evidence/SUV-0065` experiment refuted the frame-proof premise it assumed, so its safe branch applies to all of them). Tickets are minted **only** through Electron main (`__pages:request-activation`), which spends a main-observed `input-event` gesture; there is no RPC equivalent and no fallback, so the WebUI cannot mutate and a request with no fresh gesture is refused before any dialog. `loadCurrentPage` re-reads `page.json` immediately before the executor — a queued action must not act on the snapshot it was admitted with — and the ticket's descriptor binding is re-checked against the reloaded grant. Cancellation is lease-bound (`cancelAction(leaseId, nonce, requestId)`); `dropLease` aborts in-flight controllers **before** deleting the lease. **Audit rows carry closed enums and bounded identifiers only.** Permitted: action kind, HTTP method, outcome/rejection codes (`PageActionOutcomeCode` and the validation codes), and bounded identifiers — `toolName` (capped at `MAX_AUDITED_IDENTIFIER_CHARS` at the writer, not just at the parse boundary) plus page/grant/request/lease/workspace ids. Forbidden: request paths, query params, MCP arguments, response bodies, script stdout/stderr, the dynamic rejection `reason`, and any `result.error` originating outside the process — those go to the RPC response and the debug log instead. Key-name redaction is NOT the guarantee: it only catches keys it recognizes, so it protects `apiToken` and not `note`, `ref`, or a path segment.
- **Scheduled refresh admission** (`pages/scheduled-admission.ts`): a cron tick has no render, so it never reaches the broker. `admitScheduledPageRefresh` is the authoritative gate on that path — origin policy, permission mode, grant existence, digest binding, expiry, descriptor identity, audited either way — called by `automations/script-executor.ts` immediately before spawn. It **decides and does not execute**: the hardened argv/no-shell runner still owns spawning, and `assertPageRefreshGrant` remains the single grant definition. No lease or nonce is synthesized for a run that has no render (`PageActionOriginPolicy.requiresRenderLease`). An absent workspace `defaults.permissionMode` resolves to the product default `ask`, **not** `safe` — reading absence as Explore would silently stop refreshes in every workspace that never set the field.
