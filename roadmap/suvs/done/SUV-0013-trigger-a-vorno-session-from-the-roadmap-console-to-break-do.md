...
- `2026-08-25` — "Sessions are inspectable in Vorno": mechanism landed under SUV-0021 (console passes `--no-cleanup`; finished sessions are archived into `~/.craft-agent-roadmap-runner/archive/`, recorded on the record's `dispatch.sessions`). The box becomes tickable after the owner's one-time step — register the archive folder in the desktop app via *Open existing folder* — and sessions then appear on the next app launch. Live (no-restart) visibility needs a product change and is written up as finding D7 in the PLAN-039 authoring-gaps discussion; this box's remainder is re-homed there.
