...
The fork's HTTP trigger server (`apps/server`) must run in two host modes per the approved internal "Inbound Webhooks & Headless Server — Design Spec" (approved by the maintainer 2026-07-06; internal spec, `vorno-internal`):
...
- Packaging changes are the fork's most fragile surface (`vorno-internal:learnings/LEARNING-011-*`, private: collector OOM, silent `extraResources` skips, mandatory staging).
...
- Zero packaging changes: the embedded host compiles into `main.cjs` via the existing esbuild step; no new binaries, no electron-builder edits, no exposure to the packaging fragility captured in `vorno-internal:learnings/LEARNING-011-*`. Packaged-build correctness is structural, then smoke-verified.
...
- **Wire up the spawn seam (bundle `apps/server` + bun, supervise a child process)** — cheaper than feared (bun already vendored; server bundles to one 14.6 MB file), but the callback seam cannot cross a process boundary, forcing a bespoke child→main forwarding protocol the spec's design explicitly avoided; sessions/settings resolve in a second process; every bundling change re-enters the packaging-fragility territory captured in `vorno-internal:learnings/LEARNING-011-*` (private). Rejected.
...
- "Inbound Webhooks & Headless Server — Design Spec" (internal spec, `vorno-internal`, approved 2026-07-06) — embedded/standalone amendment; `onWebhookEvent` seam.
- `vorno-internal:learnings/LEARNING-011-*` (private) — electron-builder collector OOM + mandatory staging (why packaging changes are the fragile surface).
...
- PLAN-013 (standalone/headless host, "Headless server mode"); related board tickets tracked internally.
