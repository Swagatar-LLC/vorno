...
### Live OpenAI `GET /v1/models` enumeration (PLAN-009)
...
- **What**: OpenAI Pi connections enumerate models live from `GET /v1/models` (filtered to chat/reasoning, enriched from the SDK catalog, `pi/`-prefixed) instead of relying on the static `@mariozechner/pi-ai` catalog. Mirrors the existing Copilot live-fetch pattern; falls back to the static catalog on failure. Includes `inferAnthropicContextWindow` so a new Opus isn't mis-sized at 200k.
