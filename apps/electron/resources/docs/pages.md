...
- Per-render caps: 5 requests in flight, **2 mutating at a time** with up to 4 more queued behind them, and 30 starts/minute. Above the render there are also ceilings a re-mount cannot reset: 60 starts/minute per page and 120 per workspace. Cancel with `{ type: 'action-cancel', requestId, nonce }`.
...
- **Keep it short: 1000 characters max.** The cap is a security control, not a style rule — the consent dialog shows the body in full, and it can only do that if the body fits. Never assume the user saw only part of it; they saw all of it, and a long wall of text is a worse ask, not a longer one.
- **The target must already exist, be in this workspace, and not be finished.** Name a real session id (`list_sessions` in your own session, or ask the user which one). There is **no "current session" default** — an id that doesn't resolve inside the page's workspace, or that names an archived or closed (`done`/`cancelled`) session, is refused before the user is even asked, so the request just fails. The approval dialog shows the session's real name, resolved by the host. A session that is merely *busy* is still grantable: busy is a moment, not a state, so it is refused at click time (`session-busy`) rather than at approval.
