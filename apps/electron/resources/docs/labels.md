...
> **CLI-first workflow (recommended):** Use `vorno-cli label ...` commands instead of editing JSON directly.
> - `vorno-cli label --help`
...
Prefer `vorno-cli` commands:

```bash
vorno-cli label create --name "Bug" --color "destructive"
vorno-cli label create --name "Priority" --color "accent" --value-type number
vorno-cli label create --name "Due Date" --color "info" --value-type date
vorno-cli label create --name "Docs" --color "info" --value-type link
vorno-cli label create --name "Project" --color "foreground/60"
vorno-cli label create --name "Alpha" --color "info" --parent-id project
vorno-cli label create --name "Beta" --color "success" --parent-id project
```
