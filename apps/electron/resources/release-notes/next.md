...
- **Sessions no longer lost across copied workspaces** — session persistence is now tracked per workspace as well as per session. Two workspaces that hold the same session id — which happens whenever a workspace is copied, restored from a backup, or cloned between machines — were treated as one session, so deleting the session in one workspace could delete the other workspace's live transcript. SUV-0066
- **Renames and read state stop reverting** — a session edited in another window, by a sync tool, or by a second running copy of Vorno no longer has that edit undone a moment later by a save already in flight. Name, labels, flag, status, permission mode, and read state are each reconciled on their own, so an edit to one no longer discards a concurrent edit to another. SUV-0066
- **Accepted plans survive a restart** — a plan accepted with "Accept & Compact" was kept only on disk, so the next save of that session deleted it and the plan never ran. It is now carried in memory for the session's whole lifetime. SUV-0066
- **Unsent draft text stays local** — text typed but not sent when accepting a plan was included in the session list sent to every connected client. It is now kept on disk for recovery only. SUV-0066
