...
- **Automations can now act on sessions from any event.** `set-status`, `set-labels`, and `send-message` used to be accepted only on `WebhookReceived`, which made most lifecycle workflows impossible to express. They now work on any automation event — add a label and have the status follow, react to a status change by relabelling, message a session when something happens to it. Target a session with `$CRAFT_SESSION_ID`. Loop protection is built in, so an action that would re-trigger its own rule stops instead of running away, and every action records who caused it in the automation's history.
- **Automations that can never run are now reported instead of failing silently.** A rule with an invented action type, a mistyped filter key, a malformed action, or a misspelled event name used to validate clean, look healthy, and do nothing forever — and a mis-keyed filter was worse than useless, because the rule would fire on *every* event of its type rather than none. Those cases now surface as errors and warnings in config validation (with "did you mean" suggestions) and as entries in the Automations history view, on both first load and hot reload after a hand-edit. Disabled rules are exempt.
...
- Moving a card into a closed column (Done, Cancelled) on the board still works with no extra confirmation — dragging it there is the decision. What changed is that automated paths no longer get that authority by accident: agents can never close a session, and an automation can only do so if its rule was registered with `allowClosed`.
...
- **Running tasks show as "In Progress" on the board again.** The built-in status set was missing `in-progress` where new workspaces are seeded, so a task the runner had actually started read back as "todo" — a board where nothing ever appeared to be running. There is now a single source of truth for the built-in statuses, and existing workspaces are repaired on load without touching statuses you authored yourself.
