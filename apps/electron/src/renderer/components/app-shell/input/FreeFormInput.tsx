      ...
      >
        ...
        <div className="relative shrink-0" data-composer-actions>
          ...
          {/* 5.25 Persistent context-usage indicator (PLAN-002).
              ...
              Fed from the same `contextDisplay` the warning badge below uses
              (upstream #1043), so the bar and the percent can never disagree.
              `limitTokens` is already the snapshot's own denominator, falling
              back to the model's window when no snapshot has arrived. */}
          {!compactMode && (
            <ContextUsageIndicator
              used={contextDisplay.usedTokens}
              limit={contextDisplay.limitTokens}
