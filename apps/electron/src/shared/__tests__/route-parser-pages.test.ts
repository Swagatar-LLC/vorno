...
describe('pages routes', () => {
  ...
  test('Pages coexists with the Projects, Workbench, and Artifacts navigators', () => {
    const cases: Array<[string, NavigationState]> = [
      ['projects', { navigator: 'projects', details: null }],
      ['workbench', { navigator: 'workbench', details: null }],
      ['artifacts', { navigator: 'artifacts', details: null }],
      ['pages', { navigator: 'pages', details: null }],
    ]
    for (const [route, expected] of cases) {
      expect(parseRouteToNavigationState(route)).toEqual(expected)
      expect(buildRouteFromNavigationState(expected)).toBe(route)
    }
