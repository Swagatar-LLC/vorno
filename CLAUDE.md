...
- `.github/workflows/validate-pr.yml` — typecheck, shared tests (threshold-based), server tests (strict), webui tests (strict), share Worker tests (strict), Pages Worker config/test checks (strict), doc tools, i18n gates, branding gate, Headroom boundary gate, build check. **All eleven must pass.**
